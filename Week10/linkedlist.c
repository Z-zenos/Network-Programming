...
/**
 * @author          zenos
 * @brief           The source file containing all the implementations for the
 *                  XOR Linked list library
 ...
 * @return struct _xor_ll_node_* - node pointer if success or NULL if failure
 */
static struct _xor_ll_node_ *_xor_ll_create_node(const void *data, size_t size);

/**
 * @brief           Computes the next node to iterate to
 * @param prev      Previous node pointer
 * @param xor_ptr   The XOR pointer value
 * @return struct _xor_ll_node_* - Next node address
 */
static inline struct _xor_ll_node_ *_xor_ll_get_next_node (
  struct _xor_ll_node_ *prev,
  uintptr_t xor_ptr
);
 ...
 */
int xor_ll_push_tail(XOR_LL *ll_ptr, const void *data, size_t size) {
  assert(ll_ptr);

  // 1. create the node
  struct _xor_ll_node_ *node = _xor_ll_create_node(data, size);
  if (!node) {
  ...
  if (NULL == ll_ptr->head) {
    ...
    // 1. update xor_ptr to prev (current tail ptr) Xor with new (next)node
    ll_ptr->tail->xor_ptr = __XOR_LL_COMPUTE_LINK(ll_ptr->tail->xor_ptr, node);
...
int xor_ll_push_head(
  ...
  if (NULL == ll_ptr->head) {
    ...
    // 1. update xor_ptr to prev (current head ptr) Xor with new (next)node
    ll_ptr->head->xor_ptr = __XOR_LL_COMPUTE_LINK(ll_ptr->head->xor_ptr, node);
...
int xor_ll_insert_iter(
  ...
  // 1. create the node
  struct _xor_ll_node_ *new_node = _xor_ll_create_node(data, size);
  if (NULL == new_node) {
    return XOR_LL_STATUS_FAILURE_ALLOC;
  }
  ...
  // the compiler will probably optimise them away anyway
  struct _xor_ll_node_ *prev = itr_ptr->iterator_prev;
  struct _xor_ll_node_ *curr = itr_ptr->iterator_curr;
  struct _xor_ll_node_ *next = _xor_ll_get_next_node(prev, curr->xor_ptr);

  // 3. the "fun" part [NOTE: eol is end of list, sol is start of list]
  if (XOR_LL_INSERTION_BEFORE_ITER == position) {
    /* Insert between prev and curr */
    const uint8_t is_sol = (NULL == prev) ? (__XOR_LL_TRUE) : (__XOR_LL_FALSE);
    ...
    if (__XOR_LL_TRUE == is_sol) {
      ...
      struct _xor_ll_node_ *prev_prev = _xor_ll_get_next_node(curr, prev->xor_ptr);
    ...
    const uint8_t is_eol = (NULL == next) ? (__XOR_LL_TRUE) : (__XOR_LL_FALSE);
    ...
    if (__XOR_LL_TRUE == is_eol) {
      ...
      struct _xor_ll_node_ *next_next = _xor_ll_get_next_node(curr,next->xor_ptr);
 ...
 */
int xor_ll_iterate_htt(XOR_LL *ll_ptr, XOR_LL_ITERATOR *itr_ptr) {
  assert(ll_ptr);
  // emtpy list
  if (NULL == ll_ptr->head) {
    assert(NULL == ll_ptr->tail);
  ...
  if (__XOR_LL_TRUE == itr_ptr->just_deleted) {
    ...
    return (itr_ptr->iterator_curr) ? (XOR_LL_STATUS_SUCCESS) : (XOR_LL_STATUS_EOL);
  }
  ...
  if (NULL == itr_ptr->iterator_curr) {
    ...
    //  is prev Xor with xor_ptr
    struct _xor_ll_node_ *curr_node = itr_ptr->iterator_curr;
 ...
 *                  XOR_LL_STATUS_EOL - the end of the list is reached
 */
int xor_ll_iterate_tth(XOR_LL *ll_ptr, XOR_LL_ITERATOR *itr_ptr) {
  assert(ll_ptr);
  // emtpy list
  if (NULL == ll_ptr->tail) {
  ...
  if (NULL == itr_ptr->iterator_curr) {
    ...
    //  is prev Xor with xor_ptr
    struct _xor_ll_node_ *curr_node = itr_ptr->iterator_curr;
    itr_ptr->iterator_curr = _xor_ll_get_next_node(itr_ptr->iterator_prev,itr_ptr->iterator_curr->xor_ptr);
    itr_ptr->iterator_prev = curr_node;
  }
 ...
 */
void xor_ll_reset_iterator(XOR_LL_ITERATOR *itr_ptr) {
  itr_ptr->node_data.ptr = NULL;
  itr_ptr->node_data.size = 0;
 ...
 */
int xor_ll_remove_node_iter(XOR_LL *ll_ptr, XOR_LL_ITERATOR *iter_ptr) {
  struct _xor_ll_node_ *del_ptr = iter_ptr->iterator_curr;
  ...
  if (!ll_ptr->head && !ll_ptr->tail) {
    ...
    struct _xor_ll_node_ *next = _xor_ll_get_next_node(NULL,iter_ptr->iterator_curr->xor_ptr);
    struct _xor_ll_node_ *next_next = _xor_ll_get_next_node(ll_ptr->head,next->xor_ptr);
    ...
    struct _xor_ll_node_ *prev = _xor_ll_get_next_node(NULL,iter_ptr->iterator_curr->xor_ptr);
    struct _xor_ll_node_ *prev_prev = _xor_ll_get_next_node(ll_ptr->tail,prev->xor_ptr);
    ...
    struct _xor_ll_node_ *next = _xor_ll_get_next_node(
      iter_ptr->iterator_prev,
      iter_ptr->iterator_curr->xor_ptr
    );

    struct _xor_ll_node_ *next_next = _xor_ll_get_next_node(
      iter_ptr->iterator_curr,
      next->xor_ptr
    );

    struct _xor_ll_node_ *prev = iter_ptr->iterator_prev;
    struct _xor_ll_node_ *prev_prev = _xor_ll_get_next_node(
      iter_ptr->iterator_curr,
      prev->xor_ptr
    );
 ...
 */
int xor_ll_pop_head(XOR_LL *ll_ptr, XOR_LL_NODE_DATA *data) {
  assert(ll_ptr);
 ...
 *                  XOR_LL_STATUS_EMPTY_LIST - empty list
 */
int xor_ll_pop_tail(XOR_LL *ll_ptr, XOR_LL_NODE_DATA *data) {
  assert(ll_ptr);
  // 1. check for empty list
  if (NULL == ll_ptr->head) {
    assert(NULL == ll_ptr->tail);
 ...
 */
int xor_ll_destroy(XOR_LL *ll_ptr) {
  assert(ll_ptr);
  // iterate through the linked list, and free all the memory allocated
  struct _xor_ll_node_ *curr = ll_ptr->head, *prev = NULL;
  while (curr) {
    // update the next and prev pointers
    struct _xor_ll_node_ *temp = curr;
...
static struct _xor_ll_node_ *_xor_ll_create_node(const void *data, size_t size) {
  ...
  struct _xor_ll_node_ *ret = malloc(sizeof(*ret));
  if (ret) {
...
static inline struct _xor_ll_node_ *_xor_ll_get_next_node(
  ...
  return (struct _xor_ll_node_ *)(__XOR_LL_COMPUTE_LINK(prev, xor_ptr));
}
